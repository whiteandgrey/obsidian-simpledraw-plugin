...
export class SimpleDrawView extends TextFileView {
    ...
    buildDOM(): void {
        ...
        contentEl.empty();
        contentEl.addClass('simpledraw-view-content');
    ...
    showDirectionMenu(): void {
        ...
        const label = this.directionMenuEl.createDiv({ cls: 'sd-dir-menu-label' });
        ...
        for (const d of dirs) {
            ...
            btn.title = d.dir;
            btn.classList.toggle('is-active', this.engine.arrowDirection === d.dir);
    ...
    updateDirectionButtons(): void {
        for (const [dir, btn] of this.directionBtns) {
            btn.classList.toggle('is-active', this.engine.arrowDirection === dir);
        }
    }
    ...
    updateMenuButtons(): void {
        ...
        this.btnInsertTextbox.classList.toggle('is-active', isTextboxMode);
        this.btnInsertArrow.classList.toggle('is-active', isArrowMode);
    }

    updateSnapButton(): void {
        const on = this.settings.snapEnabled;
        this.btnSnapToggle.classList.toggle('is-active', on);
        this.btnSnapToggle.setCssProps({ opacity: on ? '1' : '0.5' });
    }
    ...
    startEditingTextbox(id: string): void {
        ...
        alignGroup.className = 'sd-align-group';
        ...
        const updateAlignHighlights = () => {
            for (let i = 0; i < vAligns.length; i++) {
                ...
                if (btn) btn.classList.toggle('is-active', el.vAlign === vAligns[i]!.value);
            }
            for (let i = 0; i < hAligns.length; i++) {
                const btn = hAlignBtns[i];
                if (btn) btn.classList.toggle('is-active', el.hAlign === hAligns[i]!.value);
            }
        };
        ...
        const updateWmBtn = () => {
            wmBtn.classList.toggle('is-active', (el.writingMode ?? 'horizontal-tb') === 'vertical-rl');
        };
        updateWmBtn();
        ...
        const updateShapeHighlights = () => {
            ...
            for (let i = 0; i < shapes.length; i++) {
                shapeBtns[i]!.classList.toggle('is-active', shapes[i]!.value === cur);
            }
        };
        ...
        confirmBtn.addClass('sd-confirm-btn');
        ...
        const sizeDisplay = toolbar2.createSpan({ cls: 'sd-size-display' });
    ...
    startArrowLabelEditor(id: string): void {
        ...
        const sizeDisplay = toolbar.createSpan({ cls: 'sd-size-display' });
        ...
        for (const p of positions) {
            ...
            posBtn.classList.toggle('is-active', p.key === currentPos);
